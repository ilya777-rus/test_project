...
# import re
#
# # Компилируем регулярное выражение заранее
# error_regex = re.compile(r'ERROR')
#
# def sm_GetUrl(fetchurl, fetch='raw'):
#     try:
#         # Используем метод get из библиотеки requests
#         response = requests.get(fetchurl)
#         response.raise_for_status() # Проверяем, был ли получен успешный ответ
#
#         # Декодируем байтовый объект, если fetch == 'json'
#         if fetch == 'json':
#             data = response.json()
#         else:
#             data = response.text
#
#         # Проверяем, содержит ли первый элемент списка строку "ERROR"
#         if isinstance(data, bytes):
#           data = data.decode('utf-8')
#         success = not bool(error_regex.search(data))
#
...
#         # Обрабатываем ошибки декодирования ответа
#         success = False
#         data = [response.text]
#
#     return success, data
...
# logon = 'qwert123'
# # start = '2022-04-23T00:00:00.000Z'
# start = '2012-01-01T00:00:00.000Z'
# extent = 3600
# flagstring = 'all'
# dc={'FORMAT':'list'}
# status, data = SuperMAGGetIndices(logon, start, extent, flagstring,**dc)
#
# print('status', status)  # например, 'success'
#
# print(data) # pandas
...
# logon = 'qwert123'
# start=[2012,1,1,0,0,0] # alt: start='2019-11-15T10:40'
...
#  # print(i,stations[i])
#  status, data = SuperMAGGetData(logon, start, 60, flagstring, stations[i], **dc)
#  # print(data[0]['glon'],data[0]['glat'])
#  try:
#     points_api.append([data[0]['glon'],data[0]['glat']])
#     print([data[0]['glon'], data[0]['glat']])
#     # print(data)
#  except:
#    print(i,data)
...
#
# def get_data_for_station(station, points_api):
#
...
#       with mutex:
#          points_api.append([data[0]['glon'],data[0]['glat']])
#          # print(i,[data[0]['glon'], data[0]['glat']])
#     # print(data)
#     except:
#         print("EROOORRRR")
#     # mutex.release()
...
# threads = []
# for i in range(len(stations)-1):
#     t = threading.Thread(target=get_data_for_station, args=(stations[i], points_api))
#     threads.append(t)
#     t.start()
#     time.sleep(0.1)
#
...
#     status, data = SuperMAGGetData(logon, start, 60, flagstring, station, **dc)
#     try:
#       return [data[0]['glon'],data[0]['glat']]
#     except:
#       print("ERRRR")
#
# with ThreadPoolExecutor(max_workers=10) as executor:
#     futures = [executor.submit(get_data, station) for station in stations]
#     points_api = [future.result() for future in futures]
...
#     try:
#       with queue_lock:
#         points_queue.put([data[0]['glon'],data[0]['glat']])
#         print(station, [data[0]['glon'],data[0]['glat']])
#     except:
#         print('errrr')
#     time.sleep(1)
#
...
# station = "A02"
#
# status, data = SuperMAGGetData(logon, start, extent, flagstring, station, **dc)
